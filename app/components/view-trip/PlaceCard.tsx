...
export function PlaceCard({ place, location }: PlaceCardProps) {
  ...
  return (
    <div className="border-2 border-gray-400 w-full sm:w-[400px] rounded-2xl p-2 grid grid-cols-[100px_1fr] items-center">
      ...
      <div className="px-2 sm:px-4 flex flex-col gap-0 sm:gap:2 self-start">
        <div className="font-semibold leading-tight">{place.place_name}</div>
        <div className="text-[.9rem] leading-tight">{place.activity}</div>
        <div className="text-[.9rem]">&#x231B; {place.time_to_spend}</div>
      </div>
    </div>
  );
}
